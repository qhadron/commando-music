...
module.exports = class Queue extends EventEmitter {
	...
	async _checkQueue() {
		...
		setImmediate(() => this._tasks.length && this._checkQueue);
	}
	...
	_exec({ task, name } = { task: null, name: null }) {
		...
		if (task) {
			...
			if (this._tasks.length === 0) {
				...
				this._tasks.push({ task: () => {}, name: 'Status Check', p });
			}
		}
		this._checkQueue();
		return p;
	...
	async enqueue(songs, author = null) {
		if (!(songs instanceof Array)) {
			songs = [songs];
		}
		if (!songs.every(song => song instanceof Song)) {
			throw new TypeError(`Tried to enqueue an invalid song in ${songs}`);
		}
				...
				this._exec({
					task: async () => {
						if (this._q.length >= MAX_QUEUE_SIZE) {
							let prefix = author ? `${formatMention(author)}, the` : 'The';
							this.textChannel.send(
								`${prefix} limit of ${MAX_QUEUE_SIZE} is already reached.`
							);
							return Promise.reject(false);
						}
						if (author) song.author = author.username;
						let pos = this._q.push(song) + (this._isPlaying ? 1 : 0);
						return { pos, song };
					},
		...
		if (infos.length === 1) {
			let { pos, song } = infos[0];
			return this.textChannel.send(
				oneLine`
					${author ? `${formatMention(author)} added` : `Added`}
					${formatTitle(song.title)} to #${pos} in the queue.`
			);
		} else {
			let message = '';
			message += author ? `${formatMention(author)} added` : `Added`;
			message += ':\n\n';
			message += infos
				.map(({ pos, song }) => `\t${formatTitle(song.title)} to #${pos} in the queue`)
				.join(',\n');
			message += '.';
	...
	_playNext() {
		return this._exec({
			task: async () => {
				...
				if (this._isPlaying) {
					logger.log(`Already playing ${this._src && this._src.title}`);
					return;
				}
				...
				sendNowPlaying(this.textChannel, this._src);
			},
			name: `Play Next`
		});
	}
	...
	_onSongEnd() {
		return this._exec({
			task: async () => {
				...
				this._src = null;
				if (this._q.length > 0) {
					// this delay is required to fix a bug on linux:
					// this._connection would "error" (actually emit 'warn' event) with ECONNRESET and skip song
					setTimeout(() => {
						this._playNext();
					}, 100);
				} else {
